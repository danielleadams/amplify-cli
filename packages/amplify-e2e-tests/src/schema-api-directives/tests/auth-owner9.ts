import { addApi, amplifyPush, updateAuthAddUserGroups, getAwsAndroidConfig } from 'amplify-e2e-core';
...
export async function runTest(projectDir: string, testModule: any) {
  ...
  const awsconfig = getAwsAndroidConfig(projectDir);
  const userPoolId = getUserPoolId(projectDir);

  await configureAmplifyForAndroid(awsconfig);
...
async function configureAmplifyForAndroid(awsconfig: any) {
